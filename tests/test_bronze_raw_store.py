...
class BronzeRawStoreTests(unittest.TestCase):
    ...
    def test_row_payload_json_serializes_datetime_date_and_time_values(self):
        row = pd.Series(
            {
                "source_file": "data/energy_sample.xlsx",
                "machine": "印刷机024-018主機+IR",
                "datetime": datetime(2025, 6, 1, 8, 30, 0),
                "shift_date": date(2025, 6, 1),
                "shift_time": time(8, 30, 0),
                "pd_timestamp": pd.Timestamp("2025-06-01 08:30:00"),
            }
        )

        payload = json.loads(self.store._row_payload_json(row))

        self.assertEqual(payload["datetime"], "2025-06-01T08:30:00")
        self.assertEqual(payload["shift_date"], "2025-06-01")
        self.assertEqual(payload["shift_time"], "08:30:00")
        self.assertEqual(payload["pd_timestamp"], "2025-06-01T08:30:00")
